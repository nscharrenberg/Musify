...
namespace Musify_Web.Models.DAO
{
    public class AlbumDao : IAlbumContext
    {
        ...
        private Exceptions eh = new Exceptions();
        ...
        public void AddAlbum(Album album)
        {
            ...
            try
            {
                ...
                command.Parameters.Add("@created", SqlDbType.DateTime).Value =
                    DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
                command.Parameters.Add("@updated", SqlDbType.DateTime).Value =
                    DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
            ...
            try
            {
                ...
                if (dt.Rows.Count != 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        string queryFeatured =
                            "SELECT * FROM featured AS f INNER JOIN artist AS a ON a.id = f.artist_id INNER JOIN Song AS s ON s.id = f.song_id AND f.song_id = @Id";
                        ...
                        songs.Add(new Song(songId, name, number, duration, youtube, soundcloud, server, album, created,
                            updated,
                            artists));
                    }
                }
